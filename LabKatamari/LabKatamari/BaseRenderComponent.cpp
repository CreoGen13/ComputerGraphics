...
void BaseRenderComponent::Initialize()
{
	...
	viewportView_.Width = static_cast<float>(game->GetDisplay()->ClientWidth);
	viewportView_.Height = static_cast<float>(game->GetDisplay()->ClientHeight);
	viewportView_.TopLeftX = 0;
	viewportView_.TopLeftY = 0;
	viewportView_.MinDepth = 0;
	viewportView_.MaxDepth = 1.0f;

	viewportShadow_.Width = 1024.0f;
	viewportShadow_.Height = 1024.0f;
	viewportShadow_.TopLeftX = 0;
	viewportShadow_.TopLeftY = 0;
	viewportShadow_.MinDepth = 0;
	viewportShadow_.MaxDepth = 1.0f;
}
...
void BaseRenderComponent::Draw()
{
	...
	game->GetContext()->RSSetViewports(1, &viewportView_);
...
void BaseRenderComponent::PrepareFrame()
{
	...
	game->GetContext()->RSSetState(rastState_.Get());
	game->GetContext()->RSSetViewports(1, &viewportShadow_);
	game->GetContext()->IASetInputLayout(layout_.Get());
	game->GetContext()->IASetPrimitiveTopology(topologyType_);
	game->GetContext()->IASetIndexBuffer(indexBuffer_.Get(), DXGI_FORMAT_R32_UINT, 0);
	game->GetContext()->IASetVertexBuffers(0, 1, vertexBuffer_.GetAddressOf(), strides_, offsets_);
