...
CircleComponent::CircleComponent(Game* g, SimpleMath::Vector3 color) : GameComponent(g), offset()
{
	SimpleMath::Vector4 pointsTmp [TriangleCount * 6];

	for (int i = 0; i < TriangleCount; i = i++)
	{
		pointsTmp[i * 6] = SimpleMath::Vector4(0, 0, 0.5f, 1.0f),
		pointsTmp[i * 6 + 1] = SimpleMath::Vector4(color.x, color.y, color.z, 1.0f),
		pointsTmp[i * 6 + 2] = SimpleMath::Vector4(cos(2 * PI * i / TriangleCount) / 100, sin(2 * PI * i / TriangleCount) / 100, 0.5f, 1.0f),
		pointsTmp[i * 6 + 3] = SimpleMath::Vector4(color.x, color.y, color.z, 1.0f),
		pointsTmp[i * 6 + 4] = SimpleMath::Vector4(cos(2 * PI * (i + 1) / TriangleCount) / 100, sin(2 * PI * (i + 1) / TriangleCount) / 100, 0.5f, 1.0f),
		pointsTmp[i * 6 + 5] = SimpleMath::Vector4(color.x, color.y, color.z, 1.0f);
	}
...
void CircleComponent::Draw()
{
	...
	game->Context->DrawIndexed(TriangleCount * 6, 0, 0);
}
...
void CircleComponent::SetPosition(float x, float y)
{
	offset.x = x;
	offset.y = y;
}

void CircleComponent::SetPosition(DirectX::SimpleMath::Vector2 pos)
{
	offset.x = pos.x;
	offset.y = pos.y;
}

void CircleComponent::SetY(float y)
{
	offset.y = y;
}

void CircleComponent::SetX(float x)
{
	offset.x = x;
}

float CircleComponent::GetX() const
{
	return offset.x;
}

float CircleComponent::GetY() const
{
	return offset.y;
}

DirectX::SimpleMath::Vector2 CircleComponent::GetPosition() const
{
