...
BallComponent::BallComponent(Game* g) : CircleComponent(g, SimpleMath::Vector3(1.0f, 1.0f, 1.0f))
{
	lab2 = dynamic_cast<Lab2*>(g);
...
{
	if (lab2->state == PONG_STATE_NORMAL)
	{
		...
		{
			lab2->state = PONG_STATE_COOLDOWN;
			lab2->ContinueTime = std::chrono::steady_clock::now() + std::chrono::seconds(1);
			lab2->IncScore(nextPos.x > 0.0f);
			lab2->ballHits = 0;
			nextPos.x = 0.0f;
			nextPos.y = 0.0f;
			GenRndDirection();
		}
		else if (LineRectColl(GetPosition() * 800, nextPos * 800, lab2->racquetBlue->CollisionBox, false, nextPos))
		{
			SimpleMath::Vector2::Lerp(SimpleMath::Vector2(1.f, -1.f), SimpleMath::Vector2(1.f, 1.f), abs(GetY() - lab2->racquetBlue->GetY() + 0.1f) / 0.2f, Direction);
			Direction.Normalize();
			nextPos += Direction * Speed * game->DeltaTime;
			lab2->ballHits += 1;
		}
		else if (LineRectColl(GetPosition() * 800, nextPos * 800, lab2->racquetRed->CollisionBox, true, nextPos))
		{
			SimpleMath::Vector2::Lerp(SimpleMath::Vector2(-1.f, -1.f), SimpleMath::Vector2(-1.f, 1.f), abs(GetY() - lab2->racquetRed->GetY() + 0.1f) / 0.2f, Direction);
			Direction.Normalize();
			nextPos += Direction * Speed * game->DeltaTime;
			lab2->ballHits += 1;
		}
